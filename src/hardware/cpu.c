...
const struct instruction instructions[256] = {
    {"NOP", 0, nop},                    // 0x0
    {"LD BC,0x%04X", 2, ld_bc_d16},     // 0x1
    {"LD (BC),A", 0, ld_bc_a},          // 0x2
    {"INC BC", 0, inc_bc},              // 0x3
    {"INC B", 0, inc_b},                // 0x4
    {"DEC B", 0, dec_b},                // 0x5
    {"LD B,0x%02X", 1, ld_b_d8},        // 0x6
    {"RLCA", 0, rlca},                  // 0x7
    {"LD (0x%04X),SP", 2, ld_a16_sp},   // 0x8
    {"ADD HL,BC", 0, add_hl_bc},        // 0x9
    {"LD A,(BC)", 0, ld_a_bc},          // 0xa
    {"DEC BC", 0, dec_bc},              // 0xb
    {"INC C", 0, inc_c},                // 0xc
    {"DEC C", 0, dec_c},                // 0xd
    {"LD C,0x%02X", 1, ld_c_d8},        // 0xe
    {"RRCA", 0, rrca},                  // 0xf
    {"STOP 0", 0, stop_0},              // 0x10
    {"LD DE,0x%04X", 2, ld_de_d16},     // 0x11
    {"LD (DE),A", 0, ld_de_a},          // 0x12
    {"INC DE", 0, inc_de},              // 0x13
    {"INC D", 0, inc_d},                // 0x14
    {"DEC D", 0, dec_d},                // 0x15
    {"LD D,0x%02X", 1, ld_d_d8},        // 0x16
    {"RLA", 0, rla},                    // 0x17
    {"JR 0x%02X", 1, jr_r8},            // 0x18
    {"ADD HL,DE", 0, add_hl_de},        // 0x19
    {"LD A,(DE)", 0, ld_a_de},          // 0x1a
    {"DEC DE", 0, dec_de},              // 0x1b
    {"INC E", 0, inc_e},                // 0x1c
    {"DEC E", 0, dec_e},                // 0x1d
    {"LD E,0x%02X", 1, ld_e_d8},        // 0x1e
    {"RRA", 0, rra},                    // 0x1f
    {"JR NZ,0x%02X", 1, jr_nz_r8},      // 0x20
    {"LD HL,0x%04X", 2, ld_hl_d16},     // 0x21
    {"LD (HL+),A", 0, ld_hlp_a},        // 0x22
    {"INC HL", 0, inc_hl},              // 0x23
    {"INC H", 0, inc_h},                // 0x24
    {"DEC H", 0, dec_h},                // 0x25
    {"LD H,0x%02X", 1, ld_h_d8},        // 0x26
    {"DAA", 0, daa},                    // 0x27
    {"JR Z,0x%02X", 1, jr_z_r8},        // 0x28
    {"ADD HL,HL", 0, add_hl_hl},        // 0x29
    {"LD A,(HL+)", 0, ld_a_hlp},        // 0x2a
    {"DEC HL", 0, dec_hl},              // 0x2b
    {"INC L", 0, inc_l},                // 0x2c
    {"DEC L", 0, dec_l},                // 0x2d
    {"LD L,0x%02X", 1, ld_l_d8},        // 0x2e
    {"CPL", 0, cpl},                    // 0x2f
    {"JR NC,0x%02X", 1, jr_nc_r8},      // 0x30
    {"LD SP,0x%04X", 2, ld_sp_d16},     // 0x31
    {"LD (HL-),A", 0, ld_hlm_a},        // 0x32
    {"INC SP", 0, inc_sp},              // 0x33
    {"INC (HL)", 0, inc_hl},            // 0x34
    {"DEC (HL)", 0, dec_hl},            // 0x35
    {"LD (HL),0x%02X", 1, ld_hl_d8},    // 0x36
    {"SCF", 0, scf},                    // 0x37
    {"JR C,0x%02X", 1, jr_c_r8},        // 0x38
    {"ADD HL,SP", 0, add_hl_sp},        // 0x39
    {"LD A,(HL-)", 0, ld_a_hlm},        // 0x3a
    {"DEC SP", 0, dec_sp},              // 0x3b
    {"INC A", 0, inc_a},                // 0x3c
    {"DEC A", 0, dec_a},                // 0x3d
    {"LD A,0x%02X", 1, ld_a_d8},        // 0x3e
    {"CCF", 0, ccf},                    // 0x3f
    ...
    {"JP NZ,0x%04X", 2, jp_nz_a16},     // 0xc2
    {"JP 0x%04X", 2, jp_a16},           // 0xc3
    {"CALL NZ,0x%04X", 2, call_nz_a16}, // 0xc4
    {"PUSH BC", 0, push_bc},            // 0xc5
    {"ADD A,0x%02X", 1, add_a_d8},      // 0xc6
    {"RST 00H", 0, rst_00h},            // 0xc7
    {"RET Z", 0, ret_z},                // 0xc8
    {"RET", 0, ret},                    // 0xc9
    {"JP Z,0x%04X", 2, jp_z_a16},       // 0xca
    {"PREFIX CB", 0, prefix_cb},        // 0xcb
    {"CALL Z,0x%04X", 2, call_z_a16},   // 0xcc
    {"CALL 0x%04X", 2, call_a16},       // 0xcd
    {"ADC A,0x%02X", 1, adc_a_d8},      // 0xce
    {"RST 08H", 0, rst_08h},            // 0xcf
    {"RET NC", 0, ret_nc},              // 0xd0
    {"POP DE", 0, pop_de},              // 0xd1
    {"JP NC,0x%04X", 2, jp_nc_a16},     // 0xd2
    {NULL, NULL}                        // 0xd3
    {"CALL NC,0x%04X", 2, call_nc_a16}, // 0xd4
    {"PUSH DE", 0, push_de},            // 0xd5
    {"SUB 0x%02X", 1, sub_d8},          // 0xd6
    {"RST 10H", 0, rst_10h},            // 0xd7
    {"RET C", 0, ret_c},                // 0xd8
    {"RETI", 0, reti},                  // 0xd9
    {"JP C,0x%04X", 2, jp_c_a16},       // 0xda
    {NULL, NULL}                        // 0xdb
    {"CALL C,0x%04X", 2, call_c_a16},   // 0xdc
    {NULL, NULL}                        // 0xdd
    {"SBC A,0x%02X", 1, sbc_a_d8},      // 0xde
    {"RST 18H", 0, rst_18h},            // 0xdf
    {"LDH (0x%02X),A", 1, ldh_a8_a},    // 0xe0
    {"POP HL", 0, pop_hl},              // 0xe1
    {"LD (C),A", 0, ld_c_a},            // 0xe2
    {NULL, NULL}                        // 0xe3
    {NULL, NULL}                        // 0xe4
    {"PUSH HL", 0, push_hl},            // 0xe5
    {"AND 0x%02X", 1, and_d8},          // 0xe6
    {"RST 20H", 0, rst_20h},            // 0xe7
    {"ADD SP,0x%02X", 1, add_sp_r8},    // 0xe8
    {"JP (HL)", 0, jp_hl},              // 0xe9
    {"LD (0x%04X),A", 2, ld_a16_a},     // 0xea
    {NULL, NULL}                        // 0xeb
    {NULL, NULL}                        // 0xec
    {NULL, NULL}                        // 0xed
    {"XOR 0x%02X", 1, xor_d8},          // 0xee
    {"RST 28H", 0, rst_28h},            // 0xef
    {"LDH A,(0x%02X)", 1, ldh_a_a8},    // 0xf0
    {"POP AF", 0, pop_af},              // 0xf1
    {"LD A,(C)", 0, ld_a_c},            // 0xf2
    {"DI", 0, di},                      // 0xf3
    {NULL, NULL}                        // 0xf4
    {"PUSH AF", 0, push_af},            // 0xf5
    {"OR 0x%02X", 1, or_d8},            // 0xf6
    {"RST 30H", 0, rst_30h},            // 0xf7
    {"LD HL,SP+0x%02X", 1, ld_hl_sppr8},// 0xf8
    {"LD SP,HL", 0, ld_sp_hl},          // 0xf9
    {"LD A,(0x%04X)", 2, ld_a_a16},     // 0xfa
    {"EI", 0, ei},                      // 0xfb
    {NULL, NULL}                        // 0xfc
    {NULL, NULL}                        // 0xfd
    {"CP 0x%02X", 1, cp_d8},            // 0xfe
    {"RST 38H", 0, rst_38h},            // 0xff
};
